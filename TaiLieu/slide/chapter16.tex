...
\begin{huge}
\titleslide{Quyết định hợp lý (Rational decisions)}{Chương 16}
...
\blob Sở thích hợp lý

\blob Tiện ích

\blob Tiền

\blob Tiện ích đa thuộc tính

\blob Mạng quyết định

\blob Giá trị của thông tin
...
Ký hiệu:\al
\mat{$A \pref B$} \qquad \mat{$A$} được ưu tiên hơn \mat{$B$}\al
\mat{$A \indiff B$} \qquad sự thờ ơ giữa \mat{$A$} và \mat{$B$} \al
\mat{$A \prefeq B$} \qquad \mat{$B$} không được ưu tiên hơn \mat{$A$}



%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tùy chọn hợp lý}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tiếp theo là sở thích hợp lý.}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tối đa hóa tiện ích mong đợi}

\emph{Định lý} (Ramsey, 1931; von Neumann và Morgenstern, 1944):\\
...
Cách tiếp cận tiêu chuẩn để đánh giá tiện ích con người:\al
  so sánh trạng thái nhất định \mat{$A$} với xổ số tiêu chuẩn \defn{} \mat{$L_p$} có \nl
    ...
    ``thảm họa tồi tệ nhất có thể xảy ra'' \mat{$\uworst$} với xác suất \mat{$(1-p)$}\al
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Cân tiện ích}

\defn{Tiện ích chuẩn hóa}: \mat{$\ubest = 1.0$}, \mat{$\uworst = 0.0$}

\defn{Micromorts}: khả năng tử vong một phần triệu\al
  hữu ích cho roulette Nga, trả tiền để giảm rủi ro sản phẩm, v.v.

\defn{QALYs}: số năm sống được điều chỉnh theo chất lượng\al
  hữu ích cho các quyết định y tế liên quan đến rủi ro đáng kể

Lưu ý: hành vi là \emph{bất biến} w.r.t. +ve phép biến đổi tuyến tính
\mat{\[
...
Tiền không \emph{không} hoạt động như một chức năng tiện ích
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tiện ích nhóm sinh viên}
...
Thuật toán:\al
  Đối với mỗi giá trị của nút hành động \nl
    tính toán giá trị kỳ vọng của nút tiện ích cho trước hành động, bằng chứng\al
  Trả lại hành động MEU


%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tiện ích đa thuộc tính}
...
Ý tưởng 2: xác định các loại \emph{độc lập} trong preferences\\
và rút ra các dạng kinh điển cho \mat{$U(x_1,\ldots,x_n)$}

%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Sự thống trị nghiêm ngặt}
...
\mat{$\All{i} X_i(B) \geq X_i(A)$} \quad (và do đó \mat{$U(B) \geq U(A)$})
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Sự thống trị ngẫu nhiên}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tiếp theo sự thống trị ngẫu nhiên.}

Sự thống trị ngẫu nhiên thường có thể được xác định mà không cần \\
phân phối chính xác bằng cách sử dụng lý luận \emph{định tính}

Ví dụ: chi phí xây dựng tăng theo khoảng cách từ thành phố\nl
      \mat{$S_1$} gần thành phố hơn \mat{$S_2$}\al
  \mat{$\implies$} \mat{$S_1$} chiếm ưu thế một cách ngẫu nhiên \mat{$S_2$} về chi phí

Ví dụ: thương tích tăng theo tốc độ va chạm

Có thể chú thích mạng lưới niềm tin với thông tin thống trị ngẫu nhiên:\al
  \mat{$X \qplus Y$} (\mat{$X$} ảnh hưởng tích cực đến \mat{$Y$}) có nghĩa là \al
  Đối với mọi giá trị \mat{$\mbf{z}$} của cha mẹ khác của \mat{$Y$} \mat{$\mbf{Z}$} \nl
    \mat{$\All{x_1,x_2} x_1 \geq x_2 \implies
      \pv(Y|x_1,\mbf{z})$} chiếm ưu thế ngẫu nhiên \mat{$ \pv(Y|x_2,\mbf{z})$}


%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Gán nhãn các cung + hoặc --}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Cấu trúc ưu tiên: Xác định}

\mat{$X_1$} và \mat{$X_2$} \defn{tốt nhất là độc lập} của \mat{$X_3$} iff\al
  ưu tiên giữa \mat{$\< x_1,x_2,x_3 \>$} và \mat{$\< x_1',x_2',x_3 \>$}\al
  không phụ thuộc vào \mat{$x_3$}

Ví dụ: \mat{$\<Noise,Cost,Safety\>$}:\al
  \mat{$\<$}20.000 người bị ảnh hưởng, {\DollarSign}4,6 tỷ, 0,06 người chết/mpm\mat{$\>$} so với\al
  \mat{$\<$}70.000 người bị ảnh hưởng, {\DollarSign}4,2 tỷ, 0,06 người chết/mpm\mat{$\>$}

\emph{Định lý} (Leontief, 1947): nếu mọi cặp thuộc tính là P.I. phần bổ sung của nó,
thì mọi tập hợp con của các thuộc tính là P.I của phần bù của nó: \defn{P.I tương hỗ }.

\emph{Định lý} (Debreu, 1960): P.I. Hàm giá trị \mat{$\implies$} \mat{$\exists$} \defn{additive}:
\mat{\[
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Cấu trúc ưu tiên: Stochastic}

Cần cân nhắc ưu tiên so với xổ số:\\
\mat{$\mbf{X}$} là \defn{không phụ thuộc vào tiện ích} của \mat{$\mbf{Y}$} iff\al
...
\mat{$\implies$} \mat{$\exists$} \defn{hàm tiện ích nhân }: \al
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Giá trị của thông tin}

Ý tưởng: tính toán giá trị của việc thu thập từng bằng chứng có thể có \\
Có thể được thực hiện \emph{trực tiếp từ mạng quyết định}

Ví dụ: mua quyền khoan dầu\al
  Hai khối \mat{$A$} và \mat{$B$}, đúng một khối có dầu, trị giá \mat{$k$}\al
  Xác suất trước 0,5 mỗi xác suất, loại trừ lẫn nhau\al
  Giá hiện tại mỗi block là \mat{$k/2$}\al
  ``Tư vấn'' đưa ra khảo sát chính xác về \mat{$A$}. Giá hợp lý?

Giải pháp: tính giá trị dự kiến của thông tin\al
  = giá trị mong đợi của hành động tốt nhất dựa trên thông tin\nl
    trừ giá trị mong đợi của hành động tốt nhất không có thông tin\\
Khảo sát có thể cho biết `` dầu ở A '' hoặc '' không có dầu ở A '', \emph{vấn đề. 0,5 mỗi cái} (đã cho!)\al
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Công thức tổng quát}

Bằng chứng hiện tại \mat{$E$}, hành động tốt nhất hiện tại \mat{$\alpha$} \\
Kết quả hành động có thể xảy ra \mat{$S_i$}, bằng chứng mới tiềm năng \mat{$E_j$}
\mat{\[
  EU(\alpha|E) = \max_{a} \mysum_i\ U(S_i)\;P(S_i|E,a)
\]}
Giả sử chúng ta biết \mat{$E_j \eq e_{jk}$} thì chúng ta sẽ chọn \mat{$\alpha_{e_{jk}}$} s.t.
\mat{\[
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tính chất của VPI}

\emph{Không âm}---trong \emph{kỳ vọng}, không phải \emph{post hoc}
\mat{\[
\All{j,E} VPI_{E}(E_j)\geq 0\
\]}
\emph{Không phụ gia}---xem xét, ví dụ: nhận được \mat{$E_j$} hai lần
\mat{\[
VPI_{E}(E_j,E_k) \not= VPI_{E}(E_j) + VPI_{E}(E_k)
\]}
\emph{Không phụ thuộc vào đơn hàng}
\mat{\[
...
\mat{$\implies$} thu thập bằng chứng trở thành vấn đề quyết định \emph{tuần tự}



%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Hành vi định tính}
