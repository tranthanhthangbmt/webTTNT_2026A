...
\bodysep
    \prog{Unify-Internal}(\var{x}{\ac}\var{y}{\ac}\{\})
\fnsep
...
\bodysep
    ...
    \key{else if} \prog{Compound?}(\var{x}) \key{and} \prog{Compound?}(\var{y}) \key{then}
          \key{return} \prog{Unify-Internal}(\prog{Args}[\var{x}]{\ac}\prog{Args}[\var{y}]{\ac}\prog{Unify-Internal}(\prog{Op}[\var{x}]\ac\prog{Op}[\var{y}]\ac\var{$\theta$}))
    \key{else if} \prog{List?}(\var{x}) \key{and} \prog{List?}(\var{y}) \key{then}
          \key{return} \prog{Unify-Internal}(\prog{Rest}[\var{x}]{\ac}\prog{Rest}[\var{y}]{\ac}\prog{Unify-Internal}(\prog{First}[\var{x}]{\ac}\prog{First}[\var{y}]{\ac}\var{$\theta$}))
...
\func{Unify-Var}{\var{var{\ac}x}{\ac}\var{$\theta$}}{a substitution}
    ...
    \inputs{$\theta$}{the substitution built up so far}
\bodysep
    \key{if} $\mbox{\{}var/val\mbox{\}} \elt \theta$ 
        \key{then return} \prog{Unify-Internal}(\var{val}\ac\var{x}\ac\var{$\theta$})
    \key{else if} $\mbox{\{}x/val\mbox{\}} \elt \theta$ 
        \key{then return} \prog{Unify-Internal}(\var{var}\ac\var{val}\ac\var{$\theta$})
    \key{else if} \var{var} occurs anywhere in \var{x} \cmt{occur-check}
