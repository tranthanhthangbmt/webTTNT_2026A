...
\begin{huge}
\titleslide{Thuật toán tìm kiếm có thông tin (Informed search algorithms)}{Chương 4, Phần 1--2}
...
\blob Tìm kiếm đầu tiên tốt nhất
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tìm kiếm đầu tiên tốt nhất}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tham lam tìm kiếm}

Hàm đánh giá \mat{$h(n)$} (\emph{h}euristic)\nl
= ước tính chi phí từ \mat{$n$} đến mục tiêu gần nhất

Ví dụ: \mat{$h_{{\rm SLD}}(n)$} = khoảng cách đường thẳng từ \mat{$n$} đến Bucharest

Tìm kiếm tham lam mở rộng nút \emph{xuất hiện} gần mục tiêu nhất

%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Ví dụ về tìm kiếm tham lam}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Thuộc tính tìm kiếm tham lam}
...
\q{Hoàn thành} Không--có thể bị mắc kẹt trong các vòng lặp, ví dụ: với Oradea làm mục tiêu,\nl
...
\q{Hoàn thành} Không--có thể bị mắc kẹt trong các vòng lặp, ví dụ:\nl
Iasi $\rightarrow$ Neamt $\rightarrow$ Iasi $\rightarrow$ Neamt $\rightarrow$\\
Hoàn thành trong không gian hữu hạn với việc kiểm tra trạng thái lặp lại

\q{Thời gian} \mat{$O(b^m)$}, nhưng phương pháp phỏng đoán tốt có thể mang lại sự cải thiện đáng kể
...
\q{Space} \mat{$O(b^m)$}---giữ tất cả các nút trong bộ nhớ
...
tức là, \mat{$h(n) \leq h^*(n)$} trong đó \mat{$h^*(n)$} là chi phí \emph{true} từ \mat{$n$}.\\
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tính tối ưu của {\Astar} (bằng chứng tiêu chuẩn)}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Tính tối ưu của {\Astar} (hữu ích hơn)}
...
\q{Hoàn thành} Có, trừ khi có vô số nút có \mat{$f \leq f(G)$}
...
\q{Thời gian} Hàm mũ theo [lỗi tương đối trong \mat{$h$} $\times$ độ dài của soln.]
...
\q{Space} Giữ tất cả các nút trong bộ nhớ
...
\q{Tối ưu} Có---không thể mở rộng \mat{$f_{i+1}$} cho đến khi \mat{$f_i$} kết thúc
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Chứng minh bổ đề: Tính nhất quán}

Một phương pháp phỏng đoán là \defn{nhất quán} nếu \hfill\raisebox{-0.35\textwidth[0pt][0pt]{\epsfxsize=0.3\textwidth\epsffile{\file{figures}{consistency.ps}}}
\[
  h(n) \leq c(n,a,n') + h(n')
\]
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Các phương pháp phỏng đoán được chấp nhận}
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Các vấn đề đã được giải quyết}

Các phương pháp phỏng đoán được chấp nhận có thể được rút ra từ \emph{chính xác}\\
chi phí giải pháp của phiên bản \emph{thoải mái} của vấn đề

Nếu các quy tắc của câu đố 8 được nới lỏng để một viên gạch có thể di chuyển
\emph{bất cứ nơi nào}, sau đó \mat{$h_1(n)$} đưa ra giải pháp ngắn nhất

Nếu các quy tắc được nới lỏng để một ô có thể di chuyển tới \emph{bất kỳ ô liền kề nào
...
%%%%%%%%%%%% Slide %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\heading{Các vấn đề đã được giải quyết tiếp.}
...
Tìm kiếm đầu tiên tốt nhất tham lam mở rộng thấp nhất \mat{$h$} \al
  -- không đầy đủ và không phải lúc nào cũng tối ưu

Tìm kiếm {\Astar} mở rộng ở mức thấp nhất \mat{$g+h$}\al
  -- đầy đủ và tối ưu\al
