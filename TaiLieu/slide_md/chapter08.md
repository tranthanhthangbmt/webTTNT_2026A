...
- Vui với những câu nói

- Thế giới Wumpus trong FOL

---
## Ưu điểm và nhược điểm của logic mệnh đề

\smiley\ Logic mệnh đề là *declarative*: các đoạn cú pháp tương ứng với các sự kiện

\smiley\ Logic mệnh đề cho phép thông tin một phần/rời rạc/phủ định 
  
  (không giống như hầu hết các cấu trúc dữ liệu và cơ sở dữ liệu)

\smiley\ Logic mệnh đề là *thành phần*: 
  
  ý nghĩa của \mat{$B_{1,1}\land P_{1,2}$} được bắt nguồn từ ý nghĩa của \mat{$B_{1,1}$} và của \mat{$P_{1,2}$}

\smiley\ Ý nghĩa trong logic mệnh đề là *không phụ thuộc vào ngữ cảnh*
  
  (không giống như ngôn ngữ tự nhiên, ý nghĩa phụ thuộc vào ngữ cảnh)

\frowny\ Logic mệnh đề có khả năng biểu đạt rất hạn chế
  
  (không giống ngôn ngữ tự nhiên)
...
Trong khi logic mệnh đề giả định thế giới chứa *sự kiện*,
...
Giải thích chỉ định tham chiếu cho 
...
---
## Mô hình cho FOL: Rất nhiều!

Sự đòi hỏi trong logic mệnh đề có thể được tính toán bằng cách liệt kê các mô hình

Chúng tôi *có thể* liệt kê các mô hình FOL cho từ vựng KB nhất định:
...
\mat{$\All{x} P$} &nbsp;&nbsp;  đúng trong mô hình \mat{$m$} nếu \mat{$P$} đúng với \mat{$x$} là 

*mỗi* đối tượng có thể có trong mô hình

*Nói đại khái là*, tương đương với liên từ \note{} của \note{insantiations} của \mat{$P$}
\mat{\[
\begin{array}{rl}
...
\mat{$\Exi{x} P$} &nbsp;&nbsp;  đúng trong mô hình \mat{$m$} nếu \mat{$P$} đúng với \mat{$x$} là 

*một số * đối tượng có thể có trong mô hình

*Nói đại khái là*, tương đương với \note{phân cách} của \note{instantiations} của \mat{$P$}
\mat{\[
\begin{array}{rl}
...
---
## Một lỗi phổ biến khác cần tránh
...
---
## Tính chất của bộ định lượng

\mat{$\All{x}\All{y}$} giống với \mat{$\All{y}\All{x}$} (<u>tại sao</u>??)

\mat{$\Exi{x}\Exi{y}$} giống với \mat{$\Exi{y}\Exi{x}$} (<u>tại sao</u>??)

\mat{$\Exi{x}\All{y}$} là *không phải* giống với \mat{$\All{y}\Exi{x}$}
...
\mat{$\All{x} Likes(x,IceCream)$}  &nbsp;&nbsp;&nbsp;&nbsp;  \mat{$\lnot \Exi{x} \lnot Likes(x,IceCream)$} 

\mat{$\Exi{x} Likes(x,Broccoli)$}  &nbsp;&nbsp;&nbsp;&nbsp;  \mat{$\lnot \All{x} \lnot Likes(x,Broccoli)$} 

---
## Thú vị với những câu nói
...
Ví dụ: định nghĩa (đầy đủ) \mat{$Sibling$} theo \mat{$Parent$}:
...
\mat{$**Ask(KB,S)**$} trả về một số/tất cả \mat{$\sigma$} sao cho \mat{$KB \models S\sigma$}

---
## Cơ sở kiến thức về thế giới wumpus
...
Tiên đề "Khung"---mô tả *không thay đổi* do hành động
...
Mỗi tiên đề là "about" một *vị ngữ* (bản thân nó không phải là một hành động):
\mat{\begin{eqnarray*}
...
Truy vấn: \mat{$**Ask**(KB,\Exi{s} Holding(Gold,s))$}
...
Sau đó truy vấn \mat{$**Ask**(KB,\Exi{p} Holding(Gold,PlanResult(p,S_0)))$}

có giải pháp \mat{$\{p/[Forward,Grab]\}$}

Định nghĩa \mat{$PlanResult$} theo \mat{$Result$}:
  
   \mat{$\All{s} PlanResult([\,],s) = s$}
...
-- các đối tượng và quan hệ là những nguyên hàm ngữ nghĩa
...
-- quy ước mô tả hành động và thay đổi trong FOL
