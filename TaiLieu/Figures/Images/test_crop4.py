...
def find_caption_and_crop(path):
    ...
    # Gap < 15 pixels -> same block
    if not blocks:
        print(f"No content in {os.path.basename(path)}")
        return
    ...
    for i in range(1, len(blocks)):
        ...
        if curr_start - prev_end < 20: # less than 20 pixels gap, probably same text block
    ...
    # Also find the top content to crop out empty space at the top
    content_top = 0
    ...
    print(f"{os.path.basename(path)}: content top: {content_top}, drawing bottom: {content_bottom}, caption start: {caption_start_row}")

if __name__ == "__main__":
    files = glob.glob("*.jpg")[:20]
    for f in files:
        find_caption_and_crop(f)
